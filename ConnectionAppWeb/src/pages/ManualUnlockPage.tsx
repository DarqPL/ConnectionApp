...
export default function ManualUnlockPage() {
  ...
  const canSubmitEmail = useMemo(
    () => identifier.trim().length >= 3 && email.trim().length > 0,
    [identifier, email],
  );
  ...
  const verifyOtp = async () => {
    ...
    try {
      setLoading(true);
      await authService.verifyManualUnlockOtp(identifier.trim(), email.trim(), otp.trim());
    ...
    <div className="min-h-screen flex items-center justify-center p-6">
      <Card className="w-full max-w-md">
        <CardHeader>
          ...
          <CardDescription>
            Tài khoản của bạn đã bị khoá do chính yêu cầu của người dùng. Nếu muốn mở khoá vui lòng nhập email để xác thực.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          <div className="flex gap-2">
            {step === "email" ? (
              <Button className="flex-1" onClick={requestOtp} disabled={!canSubmitEmail || loading}>
              ...
              <>
                ...
                <Button className="flex-1" onClick={verifyOtp} disabled={loading || otp.trim().length < 6}>
                  {loading ? "Đang xác thực..." : "Xác thực OTP"}
                </Button>
              </>
            )}
          </div>
          <Button variant="link" className="w-full" onClick={() => navigate("/signin")} disabled={loading}>
            Quay lại đăng nhập
          </Button>
        </CardContent>
      </Card>
